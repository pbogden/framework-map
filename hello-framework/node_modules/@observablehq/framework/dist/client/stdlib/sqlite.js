...
class SQLiteDatabaseClient {
  ...
  async describeTables({ schema } = {}) {
    return this.query(
      `SELECT NULLIF(schema, 'main') AS schema, name FROM pragma_table_list() WHERE type = 'table'${schema == null ? "" : " AND schema = ?"} AND name NOT LIKE 'sqlite_%' ORDER BY schema, name`,
      schema == null ? [] : [schema]
    );
  }
...
function load(source) {
  return typeof source === "string" ? fetch(source).then(load) : source && typeof source.arrayBuffer === "function" ? source.arrayBuffer().then(load) : source instanceof ArrayBuffer ? new Uint8Array(source) : source;
}
